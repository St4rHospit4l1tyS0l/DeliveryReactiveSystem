...
using IConnection = Drs.Service.Transport.IConnection;
    ...
    public class ServiceClientBase
    {
        ...
        protected IObservable<T> GetResilientStream<T>(Func<IConnection, IObservable<T>> streamFactory, TimeSpan connectionTimeout)
        {
            ...
            var disconnected = firstDisconnection.Merge(subsequentConnection)
                .Select(_ => Notification.CreateOnError<T>(new Exception("La conexión fue cerrada.")))
                .Dematerialize();
        ...
        //protected static IObservable<TRes> ExecuteForConnection<TReq, TRes>(IDictionary<string, IHubProxy> dicExecutionHubProxy, TReq request, string module, string methodName)
        //{
        //    return Observable.FromAsync(
        //        () => dicExecutionHubProxy[module].Invoke<TRes>(methodName, request));
        //}

        protected static IObservable<TRes> ExecuteForConnection<TRes>(IDictionary<string, IHubProxy> dicExecutionHubProxy, string module, string methodName, params object[] request)
        {
            return Observable.FromAsync(
                () => dicExecutionHubProxy[module].Invoke<TRes>(methodName, request));
        }
